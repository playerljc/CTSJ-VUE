...
import { 
  push as conditionDirectiveStackPush, 
  pop as conditionDirectiveStackPop,
  isValid as conditionDirectiveStackPushIsValid,
  isEmpty as conditionDirectiveStackPushIsEmpty
} from '../conditionDirectiveStack';
...
export function conditionDirectiveDeal(argv) {
  const config = new Map([
    [`${DIRECT_PREFIX}if`, ifConditionDirectiveDeal],
    [`${DIRECT_PREFIX}else`, elseConditionDirectiveDeal],
    [`${DIRECT_PREFIX}else-if`, elseIfConditionDirectiveDeal],
  ]);
...
function ifConditionDirectiveDeal({ context, el, vAttrNames }) {
  ...
  // v-if 入栈
  if(!conditionDirectiveStackPushIsValid()) return false;

  const entry = getDirectiveEntry(el, `${DIRECT_PREFIX}if`);
  entry.value = execExpression(context, entry.expression);
  this.conditionDirectiveStack.push(entry);
...
function elseConditionDirectiveDeal({ context, el, vAttrNames }) {
  // 如果栈是empty
  if (this.conditionDirectiveStack.length === 0) return false;
}
