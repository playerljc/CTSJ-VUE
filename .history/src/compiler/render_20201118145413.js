...
import {reset} from '../compiler/conditionDirectiveStack';
...
export function render(el, isMount) {
  ...
  // 先清空conditionDirectiveStack
  self.conditionDirectiveStack = [];

  // 进行loopRender
  // vue实例代表的vnode
  const vnode = renderLoop.call(this, { context: this.$dataProxy, el: this.templateEl, parentVNode: null });
...
export function renderLoop({ context, el, parentVNode }) {
  ...
  if (!isComponent) {
    ...
    if (!isVueIns && isSlotNode(el)) {
      return renderSlotNode.call(this, { context, el, parentVNode });
    }

    // 如果是component元素
    if (isDynamicComponentNode(el)) {
      return renderDynamicComponentNode.call(this, { context, el, parentVNode });
    }

    if (isElementNode(el)) {
      // 是元素不是组件节点
      return renderElementNode.call(this, { context, el, parentVNode });
    }
  } else {
    // 自定义节点(Component)
    // 例如：
    ...
    //  </div>
    // </div>
    return renderComponentNode.call(this, { context, el, parentVNode });
  }
 ...
 * @return {VNode | Array<VNode>}
 */
function renderVAttr({ el, parentVNode, context, renderFun }) {
  /**
  ...
  if (hasVFor(vAttrNames)) {
    ...
    return {
      ...
      VNode: parentVNode ? parseVFor.call(
        this,
 ...
 * @return {VNode | Array<VNode>}
 */
export function renderElementNode({ context, el, parentVNode }) {
  // 合并多个文本节点为一个文本节点
  el.normalize();

  // 解析指令属性
  let { Continue, VNode } = renderVAttr.call(this, { el, parentVNode, context, renderFun: renderElementNode });
  ...
  for (let i = 0, len = el.childNodes.length; i < len; i++) {
    const VNodes = renderLoop.call(this, { context, el: el.childNodes[i], parentVNode: VNode });
    if (!VNodes) continue;
 ...
 */
export function renderTemplateNode({ context, el, parentVNode }) {
  const vAttrNames = getVAttrNames(el);
  ...
  if (vAttrNames.length) {
    ...
    // 解析v-if
    if (hasVIf(vAttrNames)) {
      conditionDirectiveDeal.call(self, { context, attrName: `${DIRECT_PREFIX}if`, el, vAttrNames });
      // parse v-if
      const display = parseVIf({ context, el, vAttrNames });
      // 如果不显示则返回null
      if (!display) {
  ...
  for (let i = 0, len = el.content.childNodes.length; i < len; i++) {
    const VNodes = renderLoop.call(this, { context, el: el.content.childNodes[i], parentVNode });
    if (!VNodes) continue;
 ...
 * </div>
 */
export function renderSlotNode({ context, el, parentVNode }) {
 ...
 *
 */
export function renderDynamicComponentNode({ context, el, parentVNode }) {
  const vAttrNames = getVAttrNames(el);
 ...
 * @return VNode | Array<VNode>
 */
export function renderComponentNode({ context, el, parentVNode }) {
  // 合并多个文本节点为一个文本节点
  el.normalize();
